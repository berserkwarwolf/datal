...
urlpatterns = patterns('',
    ...
    url(r'^(?P<id>\d+)-(?P<slug>[A-Za-z0-9\-]+).csv$', 'core.exportDataStream.views.csv',
        name='datastream_manager.csv'),
    url(r'^(?P<id>\d+)-(?P<slug>[A-Za-z0-9\-]+).html$', 'core.exportDataStream.views.html',
        name='datastream_manager.html'),
    url(r'^(?P<id>\d+)-(?P<slug>[A-Za-z0-9\-]+).xls(?:$|x$)', 'core.exportDataStream.views.xls',
        name='datastream_manager.xls'),
    url(r'^updategrid$', 'core.exportDataStream.views.updategrid',
    ...
    url(r'^category/(?P<category_slug>[A-Za-z0-9\-]+)/page/(?P<page>\d+)/$', 'microsites.search.views.action_browse',
        name='search.action_browse'),
    url(r'^invoke$', 'core.exportDataStream.views.invoke'),
)
