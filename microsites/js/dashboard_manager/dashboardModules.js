...
var DBManagerGuidMenuOption = GuidMenuOption.extend({
    ...
    , beforeDisplay: function(pEvent) {
        ...
        if (Configuration.isAccountDomain) {
            var message = gettext( "DBMAN-GETGUID-TITLE" ) + '<a href="' + Configuration.baseUri + '/" title="Junar">Junar.</a>';
        } else {
            var message = gettext( "DBMAN-GETGUID-TITLE" ) + 'Junar.'
        }
...
exportCSV = {
    ...
    sendDashboardCSV : function( pJson ){
        ...
        var lHtml = '<html><body>';
        lHtml = lHtml + '<form action="/dashboards/action_csv" method="POST">';
