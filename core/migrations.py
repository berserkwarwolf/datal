...
roles = Role.objects.filter(code__in = ['ao-account-admin', 'ao-enhancer', 'ao-publisher'])
...
roles = Role.objects.filter(code__in = ['ao-account-admin', 'ao-enhancer', 'ao-publisher', 'ao-free-user'])
privilege = Privilege.objects.get(code='workspace.can_signin')
for role in roles:
    Grant.objects.create(role=role, privilege=privilege)


roles = Role.objects.filter(code__in = ['ao-account-admin', 'ao-enhancer', 'ao-publisher', 'ao-member'])
